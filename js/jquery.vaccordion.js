	...
	var cache	= {
		...
		aux		= {
			...
			slide			: function( dir, $slices, $navNext, $navPrev, settings ) {
				...
				if( cache.idx_expanded === -1 || !settings.savePositions ) {
				if( dir === 1 && cache.current + 6 >= cache.totalSlices )
					return false;
				...
				if( dir === -1 && cache.current === 1 )
					...
					if( dir === 1 && cache.current + 6 === cache.totalSlices - 1 )
					$navNext.fadeOut();
				else
					$navNext.fadeIn();
				}
				...
				$slices.each(function(i) {
					...
					if( !settings.savePositions || cache.idx_expanded === -1 )
						...
						if( i === $nextSlice.index() ) {
							...
							itemHeight			= settings.expandedHeight;
							othersHeight		= ( settings.accordionH - settings.expandedHeight ) / ( 6 - 1 );
			...
			canSlideDown	: function( $slices, settings ) {
				var $last			= $slices.eq( cache.current + 6 - 1 );
			...
			init 		: function( options ) {
				...
				if( this.length ) {
					...
					return this.each(function() {
						...
						// each slice's height
						cache.sliceH		= Math.ceil( settings.accordionH / 6 );
						...
						// show / hide $navNext
						if( 6 < cache.totalSlices  )
							$navNext.show();

						// set the top & height for each slice.
						// also save the position of each one.
						// as we navigate, the first one in the accordion
						// will have position 1 and the last 6.
